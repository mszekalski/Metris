...
class Board {
    ...
    if (!this.paused) {
      if (e.key === "ArrowLeft") {
        for (let i = 0; i < this.piece.squares.length; i++) {
          ...
          if (
            x === 0 ||
            this.grid[Math.ceil(y / 25)][Math.ceil(x / 25) - 1] === "filled"
          ) {
            return;
          }
        }
      ...
      if (e.key === "ArrowDown") {
        for (let i = 0; i < this.piece.squares.length; i++) {
          ...
          let y = this.piece.squares[i][1];
          if (
            typeof this.grid[Math.ceil(y / 25) + 1] === "undefined" ||
  ...
  render(ctx) {
    ...
    this.pieces.forEach(piece => {
      currentPiece.squares.forEach(coordinates => {
        if (
          typeof this.grid[Math.floor(coordinates[1] / 25) + 1] ===
            "undefined" ||
          this.grid[Math.floor(coordinates[1] / 25) + 1][
            coordinates[0] / 25
          ] === "filled"
        ) {
    ...
    if (this.piece.landed) {
      ...
      this.piece.squares.forEach(coordinates => {
        ...
        this.grid[Math.floor(coordinates[1] / 25)][coordinates[0] / 25] =
          "filled";
      });
  ...
  addPiece() {
    ...
    for (let i = 0; i < piece.squares.length; i++) {
      if (
        this.grid[Math.floor(piece.squares[i][1] / 25)][
          piece.squares[i][0] / 25
        ] === "filled"
      ) {
